...
The app and console now own long-lived kind-1059 inbox subscriptions independent of the conversation widgets. Both preserve an existing non-empty kind-10050 list and publish the shared DM-capable defaults only when no list exists (or when migrating a known BitBlik-generated obsolete list). The coordinator backend never overwrites kind-10050 from its unrelated RPC/NIP-65 relay set. The console republishes the existing signed kind-10050 event unchanged to the coordinator's discovery, NIP-65/outbox, and configured inbox relays so clients such as Amethyst can actually discover the route. During migration the console also receives on every historically generated BitBlik inbox set, because Android clients may retain an older kind-10050 in their local user cache. Incoming wrappers are decrypted once into NDK's validated cache, case-bound, and the already-decrypted message is appended directly to exactly one participant lane with rumor-id deduplication. Each process also retains a rumor-id-keyed inbox snapshot so messages received before a lane mounts are replayed after its initial history load instead of being lost by a broadcast-stream race. Historical rendering still reads cache snapshots. Initial catch-up completes before independent per-relay live subscriptions start, avoiding a deduplicated one-shot request whose EOSE could close the live stream.
