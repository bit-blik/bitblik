...
O {app} é software livre e de código aberto, concebido para facilitar a troca peer-to-peer de Bitcoin por códigos {code}.\
A ideia fundamental é:
- pagar com Bitcoin em qualquer lado onde o pagamento {code} seja aceite
...
Embora esses serviços de escrow P2P sejam excelentes e devam ser usados para trocas maiores e de mais longo prazo, o {app} destina-se a ser usado como um método de pagamento rápido com códigos {code} em locais/situações onde isso seja apropriado, como lojas de self-checkout, restaurantes, compras online e até multibancos.
...
4.  **Pagamento em fiat (Maker):** O maker recebe o código {code} e introduz-no no terminal de pagamento ou no site de e-commerce.
...
O {code} é um sistema de pagamento móvel usado em {country}. Permite aos utilizadores fazer pagamentos usando um código de {codeLength} dígitos gerado pela app do seu banco. No {app}, os takers usam o {code} para pagar Bitcoin aos makers.
...
Se o maker tentar usar o código {code} e este falhar, a transação não pode prosseguir. O taker pode ter de fornecer um novo código, ou a oferta pode ser cancelada.
...
- **Se o taker fornecer um código {code} inválido ou expirado:** O maker não consegue completar o pagamento e não confirmará a receção. A troca falha, e o Bitcoin do maker é devolvido através do cancelamento da hold invoice. O taker não recebe nada.
...
Se o código {code} que tentou usar no terminal de pagamento ou no site de e-commerce era inválido ou estava expirado, deve fornecer prova da tentativa de pagamento falhada. Isto pode incluir:
- talão de código {code} inválido impresso pelo terminal de pagamento ou multibanco.
- captura de ecrã da tentativa de pagamento falhada no site de e-commerce
