...
{app} is free and open source software designed to facilitate the peer-to-peer exchange of Bitcoin for {code} codes — focused on paying at **ATMs (Multibanco)** in {country}.\
The fundamental idea is to:
- spend Bitcoin at any Multibanco ATM that accepts {code} payment
...
While those P2P escrow services are excellent and should be used for larger and longer-term trades, {app} is intended to be used as a quick payment method using {code} codes at **ATM machines (Multibanco)**, where you can withdraw cash or pay bills with the Bitcoin you hold.
...
4.  **Fiat Payment (Maker):** The Maker receives the {code} code and enters it at the **Multibanco ATM** to complete the payment or cash withdrawal.
...
{code} is a mobile payment system used in {country}. It allows users to make payments using a {codeLength}-digit code generated by their banking app, which can be entered directly at a Multibanco ATM. In {app}, Takers use {code} to pay Makers for Bitcoin.
...
If the Maker attempts to use the {code} code at the ATM and it fails, the transaction cannot proceed. The Taker might need to provide a new code, or the offer might be cancelled.
...
- **If the Taker provides an invalid or expired {code} code:** The Maker cannot complete the payment at the ATM and will not confirm receipt. The trade fails, and the Maker's Bitcoin is returned via hold invoice cancellation. The Taker receives nothing.
...
If the {code} code you tried to use at the Multibanco ATM was invalid or expired, you should provide evidence of the failed payment attempt. This could include:
- invalid {code} code receipt printed by the ATM.
- screenshot or printout of the failed payment attempt at the ATM
