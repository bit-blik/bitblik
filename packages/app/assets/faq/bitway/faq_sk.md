...
{app} je slobodný softvér s otvoreným zdrojovým kódom na P2P výmenu Bitcoinu za kódy {code} — zameraný na platby v **bankomatoch (Multibanco)** v krajine {country}.\
Základná myšlienka je:
- minúť Bitcoin v ktoromkoľvek bankomate Multibanco, ktorý akceptuje platbu {code}
- kúpiť Bitcoin generovaním a predajom kódov {code}

#### Prečo ďalší P2P nástroj? Prečo nie RoboSats, Bisq či Hodl Hodl?

Tie escrow služby sú výborné na väčšie a dlhodobejšie obchody. {app} slúži ako rýchla platobná metóda kódom {code} v **bankomatoch (Multibanco)**, kde môžeš vybrať hotovosť alebo platiť účty Bitcoinom, ktorý držíš. Celá výmena zvyčajne trvá pár minút.
...
4.  **Platba fiatom (Maker):** Maker dostane kód {code} a zadá ho v **bankomate Multibanco**, čím dokončí platbu alebo výber hotovosti.
...
{code} je mobilný platobný systém používaný v krajine {country}. Umožňuje platiť {codeLength}-miestnym kódom vygenerovaným v bankovej appke, ktorý sa dá zadať priamo v bankomate Multibanco. V {app} takeri používajú {code} na platbu makerom za Bitcoin.
...
Ak sa maker pokúsi použiť kód {code} v bankomate a zlyhá, transakcia nemôže pokračovať. Taker možno bude musieť poskytnúť nový kód, alebo sa ponuka zruší.
...
Ak bol kód {code} v bankomate Multibanco neplatný alebo vypršaný, predlož dôkaz o neúspešnom pokuse: účtenku o neplatnom kóde {code} z bankomatu alebo screenshot/výtlačok neúspešnej platby v bankomate.
